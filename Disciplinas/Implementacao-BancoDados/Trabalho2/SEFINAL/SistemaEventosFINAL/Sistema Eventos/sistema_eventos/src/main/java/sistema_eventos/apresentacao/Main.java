...
public class Main {
    public static void main(String[] args) throws SQLException {
        ...
        // ----------- LOGIN --------------
        // TELA LOGIN - GET/POST [ADMIN/ORG/PART]
        app.get("/login", ctx -> {
        ...
        app.get("/participante/evento/{id}/inscrever", ctx -> {
            ...
            DAOEvento daoEvento = new DAOEvento();
            DAOEventoPrivado daoPrivado = new DAOEventoPrivado();

            Evento evento = daoEvento.buscarPorId(eventoId);
            
            boolean eventoPrivado = daoEvento.isEventoPrivado(eventoId);
            System.out.println("DEBUG: eventoId=" + eventoId + " eventoPrivado=" + eventoPrivado);
        ...
        app.post("/participante/evento/{id}/inscrever", ctx -> {
            ...
            if (daoEvento.isEventoPrivado(eventoId)) {
                String senhaInformada = ctx.formParam("senhaAcesso");

                if (!daoEvento.verificarSenhaEventoPrivado(eventoId, senhaInformada)) {
                    ctx.html("""
                        <script>
                            alert('Senha inválida.');
                            window.location.href = '/participante/perfil';
                        </script>""");
                    return;
                }
        ...
        app.get("/participante/evento/{id}", ctx -> {
            ...
            // Busca o evento na tabela evento
            DAOEvento daoEvento = new DAOEvento();
            ...
            DAOEventoPrivado daoPrivado = new DAOEventoPrivado();
            boolean isPrivado = daoPrivado.existeEventoPrivado(eventoId);
            ...
            if (evento.getImagemBanner() != null) {
                String base64 = Base64.getEncoder().encodeToString(evento.getImagemBanner());
                model.put("bannerBase64", base64);
            }
            
            if (isPrivado) {
                ctx.render("/area/participante/evento_privado.html", model);
            } else {
                ctx.render("/area/participante/evento.html", model);
            }
        });
        ...
        app.post("/organizador/evento/{id}/palestra/cadastrar", ctx -> {
            ...
            // Usuário
            String login = ctx.formParam("loginPalestrante");
            String senha = ctx.formParam("senhaPalestrante");
            ...
            int usuarioId = daoUsuario.inserir(login, senha, 4);

            // Cria o objeto palestrante e associa o usuarioId
            Palestrante novoPalestrante = new Palestrante();
        ...
        app.post("/admin/evento-privado/novo", ctx -> {
            try {
                ...
                if (arquivo != null) {
                    byte[] imagemBanner = arquivo.content().readAllBytes();;
        ...
        app.get("/admin/evento-privado/{id}", ctx -> {
            ...
            Evento evento = daoEvento.buscarPorId(idEvento);
            EventoPrivado privado = dao.buscarPorId(idEvento);

            if (evento == null || privado == null) {
            ...
            if (evento.getDetalhes() != null && !evento.getDetalhes().isBlank()) {
                try {
                    detalhesMap = mapper.readValue(evento.getDetalhes(), Map.class);
                } catch (JsonProcessingException e) {
                    detalhesMap.put("descricao", evento.getDetalhes());
                }
            }

            List<String> listaConvidados = new ArrayList<>();
            if (privado.getListaConvidados() != null && !privado.getListaConvidados().isBlank()) {
                try {
                    listaConvidados = mapper.readValue(privado.getListaConvidados(), List.class);
                } catch (JsonProcessingException e) {
