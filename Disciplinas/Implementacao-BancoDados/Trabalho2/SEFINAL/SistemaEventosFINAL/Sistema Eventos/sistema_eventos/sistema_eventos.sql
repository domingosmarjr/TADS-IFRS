...
-- -------
-- Papel Aluno -> Usuário Aluno1
-- Criação de Papéis
--  1>ADM          (cria usuários, eventos, opera tudo)
--  2>Organizador  (cria e edita eventos, consulta eventos e insere arquivos)
--  4>Palestrante  (insere palestra e edita palestra)
--  3>Participante (consulta eventos, consulta palestra, inscreve-se em eventos, inscreve-se palestras)
...
/*
    - Administrador
        ...
        > Inscreve Participante-Evento
        > Exclui Inscricação Participante-Evento
        > Lista Inscricões
...
GRANT SELECT, INSERT, UPDATE, DELETE ON evento TO organizador;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE evento_id_seq TO organizador;
GRANT SELECT, INSERT, UPDATE, DELETE ON palestra TO organizador;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE palestra_id_seq TO organizador;

GRANT SELECT, INSERT, UPDATE, DELETE ON palestrante TO organizador;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE palestrante_id_seq TO organizador;

GRANT SELECT, INSERT, UPDATE, DELETE ON palestra TO organizador;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE palestra_id_seq TO organizador;

CREATE ROLE palestrante;
GRANT SELECT, INSERT, UPDATE ON palestra TO palestrante;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE palestra_id_seq TO palestrante;
GRANT SELECT, INSERT, UPDATE ON palestrante TO palestrante;
GRANT USAGE, SELECT, UPDATE ON SEQUENCE palestra_id_seq TO palestrante;

CREATE ROLE viewer;
GRANT SELECT ON evento TO viewer;

-- Verificar Todas Roles de Usuários
SELECT roleid::regrole, member::regrole FROM pg_auth_members;

-- CRIAÇÃO DE UM ADMIN SUPERUSUÁRIO
INSERT INTO usuario (login, senha, tipo) VALUES ('admin','admin',1);
...
CREATE TABLE evento (
    ...
    detalhes jsonb,
    imagem_banner bytea,
    organizador_id integer REFERENCES organizador(id) ON DELETE SET NULL
);

CREATE TABLE evento_privado (
    id integer PRIMARY KEY REFERENCES evento(id) ON DELETE CASCADE,
    senha_acesso character varying(100) NOT NULL,
    lista_convidados jsonb DEFAULT '[]'
); --REMOVI A HERANÇA PARA FAZER O SISTEMA FUNCIONAR = INHERITS (evento);
-- O uso da herança do Postgres estava dificultanto na hora de realizar inscrição de participante por causa da referência de PK
...
-- INSERT INTO participante (nome, data_nascimento, cpf, email) VALUES
-- ('Alice', '1980-01-01', '12345678910', 'alice@live.com'),
-- ('Angela', '1985-01-01', '12345678911', 'angela@live.com'),
-- ('Ana Lucia', '1990-01-01', '12345678912', 'analu@live.com'),
-- ('Aninha', '1995-01-01', '12345678913', 'aninha@live.com'),
-- ('Anastacia', '2000-01-01', '12345678914', 'anat@live.com'),
-- ('Bruna', '2005-01-01', '12345678915', 'bru@live.com'),
-- ('Bianca', '2007-01-01', '12345678916', 'bian@live.com'),
-- ('Bela', '2010-01-01', '12345678917', 'bela@live.com'),
-- ('Bia', '2011-01-01', '12345678918', 'bia@live.com'),
-- ('Beatriz', '2015-01-01', '12345678919', 'bea@live.com'),
-- ('Bruninha', '1996-01-09', '12345678920', 'bru@live.com'),
-- ('Belinha', '1995-02-01', '12345678921', 'bel@live.com'),
-- ('Marcio Torres','1980-10-02', '12345678922', 'marc@live.com');
